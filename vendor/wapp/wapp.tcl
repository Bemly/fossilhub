...
#
#   (2)  Identifiers intended for internal use only begin with "wappInt"
#
...
if {$tcl_version>=8.7} {
  ...
  proc wappInt-enc {all mode nu1 txt} {
    return [uplevel 2 "wapp-escape-$mode \"$txt\""]
  }
} else {
  proc wapp-subst {txt} {
    global wapp
...
# There must be a wapp-escape-NAME routine for each possible substitution
# in wapp-subst.  Thus there are routines for "html", "url", "qp", and "unsafe".
#
#    wapp-escape-html           Escape text so that it is safe to use in the
#                               body of an HTML document.
#
#    wapp-escape-url            Escape text so that it is safe to pass as an
#                               argument to href= and src= attributes in HTML.
#
#    wapp-escape-qp             Escape text so that it is safe to use as the
#                               value of a query parameter in a URL or in
#                               post data or in a cookie.
#
#    wapp-escape-string         Escape ", ', \, and < for using inside of a
#                               javascript string literal.  The < character
#                               is escaped to prevent "</script>" from causing
#                               problems in embedded javascript.
#
#    wapp-escape-unsafe         Perform no encoding at all.  Unsafe.
#
proc wapp-escape-html {txt} {
  return [string map {& &amp; < &lt; > &gt; \" &quot; \\ &#92;} $txt]
}
proc wapp-escape-unsafe {txt} {
  return $txt
}
proc wapp-escape-url {s} {
...
# Configure whether wapp will automatically convert text/* output to
# UTF-8, defaulting to on for historical compatibility. This is
# conversion is known to be faulty with certain inputs so should be
# disabled if the client can guaranty that their output data are valid
# UTF-8.
#
# Pass a value of 1 to enable, 0 to disable.
#
proc wapp-encode-text-as-utf8 {{yes 1}} {
  global wapp
  dict set wapp .text-to-utf8 $yes
}

# This is a helper routine for wapp-escape-url and wapp-escape-qp.  It returns
...
# website if the link contains a query string, by default.
#
    ...
    foreach {nm val} [dict get $wapp .new-cookies] {
      if {[regexp {^[a-z][-a-z0-9_]*$} $nm]} {
        if {$val==""} {
          ...
          set val [wapp-escape-url $val]
          puts $chan "Set-Cookie: $nm=$val; HttpOnly; Path=/\r"
        }
      }
    }
  }
  if {[wappInt-mimetype-text $mimetype]} {
    set reply [dict get $wapp .reply]
    if {1 eq [dict get $wapp .text-to-utf8]} {
      set reply [encoding convertto utf-8 $reply]
    }
