...
源码输入由已提交修订通过 `git archive` 生成。最终烟测、测试和初始化容器在验收后均已停止，并于 2026-08-29 在获得明确授权后删除；绑定数据、Docker 卷和镜像没有删除。原生产回滚容器 `fossilhub-rollback-0ac4dff-20260829` 及其数据目录也已在当天明确授权后永久删除。
...
用户随后明确要求删除旧 `0ac4dff` 回滚容器及其 1.6 MB 数据目录；两者已永久删除且无法恢复。又明确授权删除全部非生产 FossilHub init/test/beta 容器；当前生产、其他历史回滚容器、烟测数据、Docker 卷和镜像均未受影响。本地 clone 测试产物已移入桌面废纸篓，可恢复。
